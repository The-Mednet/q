-- ====================================================================
-- Provider Management Schema Migration (MySQL 5.7+ Compatible)
-- Version: 005
-- 
...
-- 
-- Features MySQL-compatible syntax with defensive programming
-- ====================================================================
...
-- Add missing columns to existing workspaces table using information_schema checks
SET @sql_enabled = (SELECT COUNT(*) FROM information_schema.columns 
    WHERE table_schema = DATABASE() AND table_name = 'workspaces' AND column_name = 'enabled');
SET @sql_created_at = (SELECT COUNT(*) FROM information_schema.columns 
    WHERE table_schema = DATABASE() AND table_name = 'workspaces' AND column_name = 'created_at');
SET @sql_updated_at = (SELECT COUNT(*) FROM information_schema.columns 
    WHERE table_schema = DATABASE() AND table_name = 'workspaces' AND column_name = 'updated_at');
SET @sql_created_by = (SELECT COUNT(*) FROM information_schema.columns 
    WHERE table_schema = DATABASE() AND table_name = 'workspaces' AND column_name = 'created_by');
SET @sql_description = (SELECT COUNT(*) FROM information_schema.columns 
    WHERE table_schema = DATABASE() AND table_name = 'workspaces' AND column_name = 'description');

SET @sql = CASE 
    WHEN @sql_enabled = 0 THEN 'ALTER TABLE workspaces ADD COLUMN enabled BOOLEAN NOT NULL DEFAULT TRUE COMMENT "Workspace active status";'
    ELSE 'SELECT "Column enabled already exists" AS status;'
END;
...
SET @sql = CASE 
    WHEN @sql_created_at = 0 THEN 'ALTER TABLE workspaces ADD COLUMN created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT "Creation timestamp";'
    ELSE 'SELECT "Column created_at already exists" AS status;'
END;
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = CASE 
    WHEN @sql_updated_at = 0 THEN 'ALTER TABLE workspaces ADD COLUMN updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Last update timestamp";'
    ELSE 'SELECT "Column updated_at already exists" AS status;'
END;
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = CASE 
    WHEN @sql_created_by = 0 THEN 'ALTER TABLE workspaces ADD COLUMN created_by VARCHAR(255) NULL COMMENT "User who created the workspace";'
    ELSE 'SELECT "Column created_by already exists" AS status;'
END;
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = CASE 
    WHEN @sql_description = 0 THEN 'ALTER TABLE workspaces ADD COLUMN description TEXT NULL COMMENT "Workspace description";'
    ELSE 'SELECT "Column description already exists" AS status;'
END;
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Add indexes (MySQL will ignore if they already exist)
ALTER TABLE workspaces ADD INDEX idx_workspaces_enabled (enabled);
ALTER TABLE workspaces ADD INDEX idx_workspaces_created (created_at);
ALTER TABLE workspaces ADD INDEX idx_workspaces_domain (domain);
...
CREATE TABLE IF NOT EXISTS workspace_rate_limits (
    ...
    CONSTRAINT fk_workspace_limits_workspace 
...
CREATE TABLE IF NOT EXISTS workspace_user_rate_limits (
    ...
    CONSTRAINT fk_workspace_user_limits_workspace 
        FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS provider_header_rewrite_rules (
    ...
    CONSTRAINT fk_header_rules_provider 
        FOREIGN KEY (provider_id) REFERENCES workspace_providers(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS workspace_domains (
    ...
    CONSTRAINT fk_workspace_domains_workspace 
        FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS provider_statistics (
    ...
    CONSTRAINT fk_provider_stats_provider 
        FOREIGN KEY (provider_id) REFERENCES workspace_providers(id) ON DELETE CASCADE,
...
-- Drop views if they exist (MySQL 5.7+ compatible)
DROP VIEW IF EXISTS workspace_complete_view;
DROP VIEW IF EXISTS provider_details_view;

-- Complete workspace view with all provider information
CREATE VIEW workspace_complete_view AS
SELECT 
...
CREATE VIEW provider_details_view AS
SELECT 
...
SELECT 
    'Migration completed successfully. Database now supports comprehensive provider management.' AS status,
...
-- ✅ Information_schema checks for existing columns before ALTER
-- ✅ Proper foreign key constraint naming
-- ✅ MySQL-compatible view creation with DROP/CREATE pattern
